...
public class PostService {
    ...
    public Long createPost(PostCreateRequest req, Long memberId, MultipartFile file) {
        ...
        if (hasFile(file)) fileUrl = fileHandler.upload(file, FileDirectory.POST_ATTACHMENT, post.getId().toString());
        post.updateFileUrl(fileUrl);
    ...
    public Long updatePost(Long postId, Long memberId, PostUpdateRequest updateRequest, MultipartFile file) {
        ...
        checkPermission(findMember, findPost);
        String fileUrl = "";
        if (hasFile(file)) fileUrl = fileHandler.upload(file, FileDirectory.POST_ATTACHMENT, postId.toString());
